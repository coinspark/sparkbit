 ...
 * 
 */
public abstract class AbstractTradePanel extends JPanel implements Viewable, CopyQRCodeImageDataProvider, BitcoinFormDataProvider {
...
//    protected Money parsedAmountFiat = null;
    ...
    /*CoinSpark*/ protected JComboBox typeComboBox;
    
    /* CoinSpark START */
    protected MultiBitTextField sparkAddressTextField;
    protected JPanel theFormPanel; // we want a reference to the form so we can switch components.
    /* CoinSpark END */
    ...
    /* CoinSpark START */
    private static final int SPARKBIT_STENT_DELTA = 20; // extra padding we want for keys on left of panel.
    /* CoinSpark END */
...
//	log.debug(">>>> received event: " + event);
	...
	// Disable QRCode for now, so don't shot the dashed stent. 
	/*
	...
	*/
    ...
    class SelectionListener implements ListSelectionListener {
        ...
        public void valueChanged(ListSelectionEvent event) {
			...
			/*CoinSpark START*/
			if (thisAbstractTradePanel.isReceiveBitcoin()) {
			    String address = addressTextField.getText();
			    if (address != null) {
				String sparkAddress = CSMiscUtils.convertBitcoinAddressToCoinSparkAddress(address);
    ...
    protected JPanel createAmountPanel() {
        ...
        amountBTCTextField.setHorizontalAlignment(JTextField.LEADING);
        ...
        constraints2.gridy = 0;
        constraints2.weightx = 0.4; // Used to be 2.0 but we don't need it so wide
	...
	// Experiment to push cells up, but than Amount label cannot be aligned.
//	constraints2.fill = GridBagConstraints.VERTICAL;
//        constraints2.gridx = 0;
//        constraints2.gridy = 1;
//        constraints2.weightx = 0.1;
//        constraints2.weighty = 1000.0;
//        constraints2.gridwidth = 1;
//        constraints2.gridheight = 1;
//        constraints2.anchor = GridBagConstraints.CENTER;
//        amountPanel.add(MultiBitTitledPanel.createStent(3, 3), constraints2);
...
//        amountEqualsLabel.setVisible(fiatIsVisible);
        ...
        /*CoinSpark*/
        //CoinSpark-Commenting out so Unit Test 'SendBitcoinConfirmActionTest' does not fail
        /*
        final MultiBitLabel tempLabel = amountUnitBTCLabel;
        typeComboBox.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                String temp = typeComboBox.getSelectedItem().toString();
                if (temp.equalsIgnoreCase(controller.getLocaliser().getString("sendBitcoinPanel.amountUnitLabel")))
                {
                ...
                tempLabel.setText(typeComboBox.getSelectedItem().toString());
            }
        });
        */
        /*CoinSpark*/
...
//    }
	...
	displayQRCode(address, null, amount, label);
    }
    
    public void displayQRCode(String address, String assetRef, String amount, String label) {
	// Do nothing.
    }
    
    public void displayQRCode_DISABLED(String address, String assetRef, String amount, String label) {
	if (qrCodeGenerator == null) {
	    ...
	    /* CoinSpark START */
	    // If the panel is trading an asset right now, we want to show QR for the asset.
	    // We invoke methods which send panel should have overriden to get asset amount and assetref.
	    BufferedImage image = null;
	    if (!isTradingAsset()) {
                if (address!=null) {
                    image = qrCodeGenerator.generateQRcode(address, amount, label);
                }
	    } else {
		amount = getAssetAmount();
		if (assetRef == null) {
		    assetRef = getAssetReference();
		}
                if (address != null) {
                    image = qrCodeGenerator.generateCoinSparkQRcode(address, assetRef, amount, label);
                }
	    }
	    /* CoinSpark END */
    ...
    public boolean processDecodedString(String decodedString, ImageIcon icon) {
	    ...
	    /* CoinSpark START */
	    boolean isCoinSpark = false;
	    String csAddress = null;
	    String btcAddress = null;
	    String assetReference = null;
	    try {
		if (uriString.startsWith("coinspark:")) {
		    isCoinSpark = true;
		    uriString = uriString.replaceFirst("coinspark:", "bitcoin:");
		    String schemeSpecificPart;
		    if (uriString.startsWith("bitcoin://")) {
			schemeSpecificPart = uriString.substring("bitcoin://".length());
		    } else if (uriString.startsWith("bitcoin:")) {
			schemeSpecificPart = uriString.substring("bitcoin:".length());
		    } else {
			...
			throw new BitcoinURIParseException("No data found after the coinspark: prefix");
		    }
		    csAddress = addressSplitTokens[0];  // may be empty!
		    CoinSparkAddress csa = CSMiscUtils.decodeCoinSparkAddress(csAddress);
		    if (csa != null) {
			btcAddress = CSMiscUtils.getBitcoinAddressStringFromCoinSparkAddress(csa);
			if (btcAddress != null) {
			    uriString = uriString.replaceFirst(csAddress, btcAddress);
			} else {
			    throw new BitcoinURIParseException("CoinSpark address does not contain a bitcoin address");
			}
		    }
		}
	    ...
	    String decodedLabel = "";
		    ...
		    /* CoinSpark START */
		    // If there is an exception, the label will be empty.
		    if (isCoinSpark) {
			if (decodedLabel.startsWith("<CSAssetRef>") && decodedLabel.contains("</CSAssetRef>")) {
			    String tmp[] = decodedLabel.split("</CSAssetRef>");
			    assetReference = tmp[0].replaceAll("<CSAssetRef>", "").trim();
			    decodedLabel = tmp[1];
			    // We should validate the asset
			    CoinSparkAssetRef ref = new CoinSparkAssetRef();
			    boolean foundAsset = false;
			    try {
				if (ref.decode(assetReference)) {
				    CSAsset c = new CSAsset();
				    c.setAssetRef(ref);
				    CSAsset asset = this.bitcoinController.getModel().getActiveWallet().CS.getAssetDB().findAsset(c);
				    if (asset != null) {
					foundAsset = true;
				    }
				}
			    ...
			    if (!foundAsset) {
				Message message = new Message("Unknown asset reference: " + assetReference);
				MessageManager.INSTANCE.addMessage(message);
				return false;
			    }
			}
            ...
            log.debug("AbstractTradePanel#processDecodedString addressString = " + addressString + ", amountString = " + amountString
                    + ", label = " + decodedLabel + ", assetReferece (if coinspark) = " + assetReference);
		...
		setAssetAmount(amountString);	// set the amount in the asset panel quantity textfield
	    }
...
//            updateFiatAmount();
	    ...
	    // Ensure panel method isTradingAsset() returns true,
	    // If icon was dragged, it doesn't update asset payer/sender charges, we must request it.
	    if (isCoinSpark) {
		selectAssetByReference( assetReference );
	    } else {
		selectAssetByReference( null );
	    }
		...
		// We paste in a string, so let's generate the icon from all the fields.
		if (isCoinSpark) {
		    displayQRCode(addressString, assetReference, amountString, decodedLabel);
		} else {
...
//    }
    ...
    /* CoinSpark START */
    // Method so QRCode code (and others) can figure out if panel is trading bitcoin or assets
