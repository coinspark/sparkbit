...
//    private MultiBitButton showTransactionsButton;
    ...
    /* CoinSpark START */
    private CSTransactionDetailsPanel transactionDetailsPanel;
    /* CoinSpark END */
...
//	CurrencyConverter.INSTANCE.addCurrencyConverterListener(this);
    ...
    /* CoinSpark START */
    // This is the original createUI, which is now adapted in createUpperPanel()
...
//    }
	...
	transactionDetailsPanel = new CSTransactionDetailsPanel(bitcoinController, mainFrame, null);
	transactionDetailsPanel.setVisible(false);
	JScrollPane detailScrollPane = new JScrollPane(transactionDetailsPanel);
	detailScrollPane.setBorder(BorderFactory.createEmptyBorder());
	detailScrollPane.setMinimumSize(new Dimension(550, 100));

	JSplitPane splitPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT, createUpperPanel(), detailScrollPane);
	...
	JPanel transactionsPanel = createTransactionsPanel();
	p.add(transactionsPanel, BorderLayout.CENTER);
...
//	TableColumn assetTypeColumn = columnModel.getColumn(walletTableModel.getColumnIndex("type"));
//	TableColumn assetDomainColumn = columnModel.getColumn(walletTableModel.getColumnIndex("domain"));
...
//        });
	...
	dateColumn.setCellRenderer(new TrailingJustifiedDateRenderer());
	...
	if (ComponentOrientation.getOrientation(controller.getLocaliser().getLocale()).isLeftToRight()) {
	    descriptionColumn.setCellRenderer(new LeadingJustifiedRenderer());
	    
	    /* CoinSpark START */
//	    assetTypeColumn.setCellRenderer(new LeadingJustifiedRenderer());
//	    assetDomainColumn.setCellRenderer(new LeadingJustifiedRenderer());
	    descriptionOfAssetChangesColumn.setCellRenderer( new LeadingJustifiedRendererForAmountLabelWithColor());
	    /* CoinSpark END */
	} else {
	    descriptionColumn.setCellRenderer(new TrailingJustifiedStringRenderer());
	    
	    /* CoinSpark START */
//	    assetTypeColumn.setCellRenderer(new TrailingJustifiedStringRenderer());
//	    assetDomainColumn.setCellRenderer(new TrailingJustifiedStringRenderer());
	    descriptionOfAssetChangesColumn.setCellRenderer( new TrailingJustifiedStringRendererForAmountLabelWithColor());
	    /* CoinSpark END */
	}

	// Amount decimal aligned
	DecimalAlignRenderer decimalAlignRenderer = new DecimalAlignRenderer();
//	btcAmountColumn.setCellRenderer(decimalAlignRenderer);
	

	FontMetrics fontMetrics = getFontMetrics(FontSizer.INSTANCE.getAdjustedDefaultFont());
	TableColumn tableColumn = statusColumn; // status
	...
	tableColumn.setMinWidth(statusWidth + STATUS_WIDTH_DELTA);
	tableColumn.setMaxWidth(statusWidth + STATUS_WIDTH_DELTA);

	tableColumn = dateColumn; // Date.
	...
	tableColumn.setMinWidth(dateWidth);

	tableColumn = descriptionColumn; // Description.
	tableColumn.setPreferredWidth(250);
	
	tableColumn = descriptionOfAssetChangesColumn;
	tableColumn.setPreferredWidth(400);
	tableColumn.setMinWidth(250);

//	tableColumn = btcAmountColumn; // Amount (BTC).
//	int amountBTCWidth = Math.max(fontMetrics.stringWidth(controller.getLocaliser().getString("walletTransactionTableColumn.btcAmount")), fontMetrics.stringWidth("00000.000000000"));
...
//	}
	...
	int dateColumnIndex = walletTableModel.getColumnIndex("date");
	
	List<TableRowSorter.SortKey> sortKeys = new ArrayList<TableRowSorter.SortKey>();
	sortKeys.add(new TableRowSorter.SortKey(dateColumnIndex, SortOrder.DESCENDING));
	...
	rowSorter.setComparator(dateColumnIndex, comparator);
...
//	table.getColumnModel().getColumn(3).setCellRenderer(decimalAlignRenderer);
	...
	public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row,
	    ...
	    label.setText(value.toString()); // + SPACER);
	    ...
	    Insets borderInsets = getBorder().getBorderInsets(label);
	    availableWidth -= (borderInsets.left + borderInsets.right);
	    FontMetrics fm = getFontMetrics(label.getFont());

	    String tip = label.getText();
	    if (fm.stringWidth(tip) > availableWidth) {
...
// End TODO
	...
	public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row,
	    ...
	    String tip = label.getText();
	    if (tip != null && fm != null && (fm.stringWidth(tip) > availableWidth)) {
		label.setToolTipText(tip);
	    } else {
		label.setToolTipText(null);
	    }
// END TODO
    ...
    class TrailingJustifiedStringRendererForAmountLabelWithColor extends TrailingJustifiedStringRenderer {
	@Override
	public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row,
		int column) {
	    Component jc = super.getTableCellRendererComponent(table, value, isSelected, hasFocus, row, column);
	    CSMiscUtils.updateForegroundColorOfAmountChangesLabel((MultiBitLabel)jc);
	    return label;
	}
    }
    
    class LeadingJustifiedRendererForAmountLabelWithColor extends LeadingJustifiedRenderer {
