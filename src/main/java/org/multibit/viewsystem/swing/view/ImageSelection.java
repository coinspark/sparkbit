...
public class ImageSelection extends TransferHandler implements Transferable {
    ...
    private File writeImageToFile() {
		...
		/* CoinSpark START */
		String theAmount = null;
		if (abstractTradePanel.isTradingAsset()) {
		    theAmount = abstractTradePanel.getAssetAmount();
		    
		    // Let's add the asset ref too.
		    String assetRef = abstractTradePanel.getAssetReference();
		    if (assetRef!=null) {
			if (areAdding) {
			    filename = filename + " - ";
			    filename = filename + assetRef;
			}
		    }
                    areAdding = true;
		} else {
		    theAmount = abstractTradePanel.getAmount();
		}
                if (theAmount != null && !"".equals(theAmount)) {
                    if (areAdding) {
                        filename = filename + " - ";
                    }
                    filename = filename + theAmount;
                    areAdding = true;
                }
		/* CoinSpark END */
