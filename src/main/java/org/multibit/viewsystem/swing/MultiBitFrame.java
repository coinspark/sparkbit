...
import org.multibit.viewsystem.swing.view.panels.CSDeveloperToolsPanel;
import org.multibit.viewsystem.swing.view.panels.CSShowAssetsPanel;
import org.multibit.viewsystem.swing.view.panels.CSHeaderPanel;
...
import static org.multibit.viewsystem.swing.view.panels.CSShowAssetsPanel.HEIGHT_DELTA;
...
public class MultiBitFrame extends JFrame implements ViewSystem, ApplicationListener, WalletBusyListener {
    /* CoinSpark START */
    MarqueePane marquee;
    JPanel assetSummaryPanel;
    JScrollPane assetTableScrollPane;
    JTable assetTable;
    WalletAssetSummaryTableModel assetSummaryTableModel;
    CSHeaderPanel csHeaderPanel;
...
//    private HelpButton availableBalanceFiatButton;
    ...
    /* CoinSpark START */
    private CSHeaderPanel headerPanel;
    /* CoinSpark END */
    ...
    private MultiBitWalletBusyAction showMigrateAssetsAction; // CoinSpark
    private MultiBitWalletBusyAction showDevelopertoolsAction; // CoinSpark
	...
	String showMultipleWalletsText = controller.getModel().getUserPreference(BitcoinModel.SHOW_MULTIPLE_WALLETS);
        if (Boolean.TRUE.toString().equals(showMultipleWalletsText)) {
...
//        CurrencyConverter.INSTANCE.addCurrencyConverterListener(this);
	...
	CSEventBus.INSTANCE.registerAsyncSubscriber(this); // getEventBus().register(this);


	headerPanel = new CSHeaderPanel();
	...
	assetSummaryTableModel = new WalletAssetSummaryTableModel(this.bitcoinController, this);
	assetSummaryPanel = headerPanel.getTablePanel();
	assetSummaryPanel.setLayout(new BorderLayout());
	assetSummaryPanel.setBackground(ColorAndFontConstants.MID_BACKGROUND_COLOR);

	//
	// Asset summary table
	//
	UIDefaults uidefs = UIManager.getLookAndFeelDefaults();
	Font tooltipFont = uidefs.getFont("ToolTip.font");
	final FontMetrics tooltipFontMetrics = getFontMetrics(tooltipFont);
	
	assetTable = new JTable(assetSummaryTableModel) {
...
//	    }
		...
		WalletAssetTableData data = assetSummaryTableModel.getRow(row);		
		Object value = assetSummaryTableModel.getValueAt(row, col);
		if (value instanceof HashMap) {
		    HashMap<String, Object> map = (HashMap<String, Object>) value;
		    tip = (String)map.get("tooltip");
		}
		if (tip != null) {
		    int tipWidth = tooltipFontMetrics.stringWidth(tip);
		    tipWidth += 100; // fudge factor, as calculation always seems too small
		    if (tipWidth > 600) {
			tipWidth = 600; // our maximum for a tooltip
		    }
		    if (tipWidth > 0) {
			tip = "<html><p width=\"" + tipWidth + "\">" + tip + "</p></html>";
		    }
		}
		return tip;
	    }
	...
	Font assetTableFont = FontSizer.INSTANCE.getAdjustedDefaultFontWithDelta(0); //ColorAndFontConstants.MULTIBIT_DEFAULT_FONT_SIZE/2);
	FontMetrics fontMetrics = getFontMetrics(assetTableFont);

	assetTable.setBorder(BorderFactory.createEmptyBorder(0, 0, 0, 0));
	assetTable.setShowGrid(false);
	assetTable.setBackground(ColorAndFontConstants.MID_BACKGROUND_COLOR);
	assetTable.setFillsViewportHeight(true);
	assetTable.getTableHeader().setPreferredSize(new Dimension(0, 0));   // Hides header
	assetTable.setRowHeight(fontMetrics.getHeight() + 10);	// 10 is custom, change it if need be.
	assetTable.setFont(assetTableFont);    // But cells override...?
	assetTable.setSelectionBackground(assetTable.getBackground()); // no highlight on selection
	//assetTable.setRowSelectionAllowed(false); // this stops selection
	//assetTable.setCellSelectionEnabled(false);
	assetTable.setFocusable(false); // this disables cell focus
	
	// Setting the preferred size causes it be set explicitly to that size in the scrollpane,
	// so to avoid empty areas you have scroll around, let table figure out its own size.
	//assetTable.setPreferredSize(new Dimension(600, 400));
	
	//
	// Asset table mouse event handling
	//
	/*
	assetTable.addMouseListener(new MouseAdapter() {
	    //@Override
	    public void mouseClicked(MouseEvent e) {
		int r = assetTable.rowAtPoint(new Point(e.getX(), e.getY()));
		int c = assetTable.columnAtPoint(new Point(e.getX(), e.getY()));
		int row = assetTable.convertRowIndexToModel(r);
		int col = assetTable.convertColumnIndexToModel(c);
	    }

	    @Override
	    public void mouseEntered(MouseEvent e) {
		//marquee.stopAutoScrolling();
	    }

	    @Override
	    public void mouseExited(MouseEvent e) {
		//marquee.startAutoScrolling();
	    }
	});
	*/
	
	//TableCellRenderer defaultIconRenderer = assetTable.getDefaultRenderer(Icon.class);
	...
	int amountBTCWidth = fontMetrics.stringWidth("00000.0000000 BTC");
	column.setPreferredWidth(amountBTCWidth);
	column.setMaxWidth(amountBTCWidth * 2);
	column.setMinWidth(amountBTCWidth);
	
	//
	// CONFIRMED COLUMN
	//
	column = columnModel.getColumn(WalletAssetSummaryTableModel.getIndexOfColumn(WalletAssetSummaryTableModel.COLUMN_SPENDABLE));
	Font f = new Font(null, Font.ITALIC, 12);
	column.setCellRenderer(new CSCustomFontDefaultTableCellRenderer(f));
	...
	// NEW ICON COLUMN
	//
	column = columnModel.getColumn(WalletAssetSummaryTableModel.getIndexOfColumn(WalletAssetSummaryTableModel.COLUMN_CONFIRMATION));
	CSButtonColumnCellRenderer newIconCellRenderer = new CSButtonColumnCellRenderer(assetTable, null);
	column.setCellRenderer(newIconCellRenderer);
	column.setMinWidth(BUTTON_ICON_COLUMN_WIDTH);
	...
	//
	// CONTRACT LINK COLUMN
	//
	//assetTable.getColumnModel().getColumn(1).setCellRenderer(defaultIconRenderer);
	column = columnModel.getColumn(WalletAssetSummaryTableModel.getIndexOfColumn(WalletAssetSummaryTableModel.COLUMN_CONTRACT));
	Action contractURLAction = new CSOpenAssetContractURLAction();
	CSButtonColumnCellRenderer buttonIcon = new CSButtonColumnCellRenderer(assetTable, contractURLAction);
	column.setCellRenderer(buttonIcon);
	column.setCellEditor(buttonIcon);
	column.setMinWidth(BUTTON_ICON_COLUMN_WIDTH);
	column.setMaxWidth(BUTTON_ICON_COLUMN_WIDTH);
	column.setWidth(BUTTON_ICON_COLUMN_WIDTH);
	column.setPreferredWidth(BUTTON_ICON_COLUMN_WIDTH);
	...
	//
	//assetTable.getColumnModel().getColumn(1).setCellRenderer(defaultIconRenderer);
	column = columnModel.getColumn(WalletAssetSummaryTableModel.getIndexOfColumn(WalletAssetSummaryTableModel.COLUMN_REFRESH));
	Action refreshAssetAction = new CSRefreshAssetAction(bitcoinController, headerPanel, this);
	CSButtonColumnCellRenderer refreshButton = new CSButtonColumnCellRenderer(assetTable, refreshAssetAction);
	column.setCellRenderer(refreshButton);
	column.setCellEditor(refreshButton);
	column.setMinWidth(BUTTON_ICON_COLUMN_WIDTH);
	column.setMaxWidth(BUTTON_ICON_COLUMN_WIDTH);
	column.setWidth(BUTTON_ICON_COLUMN_WIDTH);
	column.setPreferredWidth(BUTTON_ICON_COLUMN_WIDTH);
/*	
	...
	//
	//assetTable.getColumnModel().getColumn(1).setCellRenderer(defaultIconRenderer);
	column = columnModel.getColumn(WalletAssetSummaryTableModel.getIndexOfColumn(WalletAssetSummaryTableModel.COLUMN_DESCRIPTION));
	CSButtonColumnCellRenderer descriptionButton = new CSButtonColumnCellRenderer(assetTable, null);
	column.setCellRenderer(descriptionButton);
	//column.setCellEditor(descriptionButton);  // we don't want any click action, so don't set cell editor.
	column.setMinWidth(BUTTON_ICON_COLUMN_WIDTH);
	column.setMaxWidth(BUTTON_ICON_COLUMN_WIDTH);
	column.setWidth(BUTTON_ICON_COLUMN_WIDTH);
	column.setPreferredWidth(BUTTON_ICON_COLUMN_WIDTH);
	...
	//
	//assetTable.getColumnModel().getColumn(1).setCellRenderer(defaultIconRenderer);
	column = columnModel.getColumn(WalletAssetSummaryTableModel.getIndexOfColumn(WalletAssetSummaryTableModel.COLUMN_ISSUER));
	CSButtonColumnCellRenderer issuerButton = new CSButtonColumnCellRenderer(assetTable, null);
	column.setCellRenderer(issuerButton);
//	column.setCellEditor(issuerButton);
	column.setMinWidth(BUTTON_ICON_COLUMN_WIDTH);
	column.setMaxWidth(BUTTON_ICON_COLUMN_WIDTH);
	column.setWidth(BUTTON_ICON_COLUMN_WIDTH);
	column.setPreferredWidth(BUTTON_ICON_COLUMN_WIDTH);
*/	
	...
	//
	//assetTable.getColumnModel().getColumn(1).setCellRenderer(defaultIconRenderer);
	column = columnModel.getColumn(WalletAssetSummaryTableModel.getIndexOfColumn(WalletAssetSummaryTableModel.COLUMN_EXPIRY));
	CSButtonColumnCellRenderer expiryButton = new CSButtonColumnCellRenderer(assetTable, null);
	expiryButton.setFont(assetTableFont);
	column.setCellRenderer(expiryButton);
//	column.setCellEditor(expiryButton);
//	expiryButton.setHorizontalTextPosition(SwingConstants.LEFT);
	expiryButton.setHorizontalAlignment(SwingConstants.LEFT);
	column.setMaxWidth(300);
	//column.setWidth(iconWidth);
	...
	// NAME COLUMN
	//
	column = columnModel.getColumn(WalletAssetSummaryTableModel.getIndexOfColumn(WalletAssetSummaryTableModel.COLUMN_NAME));
//	column.setMaxWidth(300);
//	column.setWidth(300);
	int x = fontMetrics.stringWidth("01234567890123456domainname0123456789"); // short name is max 16 chars. while domain can be upto COINSPARK_GENESIS_DOMAIN_NAME_MAX_LEN = 32
	//column.setPreferredWidth(150);
	// FUDGE: make it bigger for upload to server message
	x = x+100;
	//column.setMaxWidth(x);
	column.setMinWidth(x);
	column.setWidth(x);
	Font defaultFont = assetTableFont;
	CSOpenAssetDomainHomePageAction action2 = new CSOpenAssetDomainHomePageAction();
	CSOpenAssetWebPageAction action = new CSOpenAssetWebPageAction();
	CSDualStyledLabelCellRenderer dualStyleRenderer = new CSDualStyledLabelCellRenderer(
		assetTable,
		defaultFont,
		new StyleRange(0, -1, defaultFont.getStyle(), Color.BLUE.darker(), StyleRange.STYLE_UNDERLINED),//defaultFont.getStyle()),
		// We want to show italic, but Windows font rendering problem, edge is cut off.
		new StyleRange(0, -1, Color.GRAY),
		//new StyleRange(0, -1, Font.ITALIC, Color.GRAY),
		action,
		action2
	);
	column.setCellRenderer(dualStyleRenderer);
	column.setCellEditor(dualStyleRenderer);

	
	//
	// Marquee containing the asset summary table
	//
//	marquee = new MarqueePane(assetTable);
//        marquee.setScrollDirection(MarqueePane.SCROLL_DIRECTION_UP);
//	marquee.setScrollAmount(1);
//        marquee.setScrollDelay(200);
//        marquee.setStayDelay(3000);
//        marquee.setPreferredSize(new Dimension(800, 100));
//        //marquee.setBorder(BorderFactory.createCompoundBorder(new JideTitledBorder(new PartialEtchedBorder(PartialEtchedBorder.LOWERED, PartialSide.NORTH), "Scroll Down", JideTitledBorder.LEADING, JideTitledBorder.ABOVE_TOP), BorderFactory.createEmptyBorder(0, 0, 0, 0)));	
//	marquee.setBorder(BorderFactory.createEmptyBorder(0, 0, 0, 0));
//	marquee.setBackground(ColorAndFontConstants.MID_BACKGROUND_COLOR);
//    
//	assetSummaryPanel.add(marquee, BorderLayout.CENTER);
...
//        contentPane.add(headerPanel, constraints);
	...
	assetTableScrollPane =new JScrollPane(assetTable);
	//assetTableScrollPane.setComponentOrientation(ComponentOrientation.RIGHT_TO_LEFT);
        contentPane.add(assetTableScrollPane, constraints);
	...
	// CoinSpark: FIXED POSITION 3
	/*CoinSpark START */
	JPanel assetsPanel = new JPanel(new BorderLayout());
        Viewable assetsView = viewFactory.getView(View.COINSPARK_ASSETS_VIEW);
        assetsPanel.add((JPanel) assetsView, BorderLayout.CENTER);
        viewTabbedPane.addTab(assetsView.getViewTitle(), assetsView.getViewIcon(), assetsView.getViewTooltip(),
                assetsPanel);
	
	// CoinSpark: FIXED POSITION 4
//	JPanel debugPanel = new JPanel(new BorderLayout());
//        Viewable debugView = viewFactory.getView(View.COINSPARK_DEVELOPER_TOOLS_VIEW);
//        debugPanel.add((JPanel) debugView, BorderLayout.CENTER);
//        viewTabbedPane.addTab(debugView.getViewTitle(), debugView.getViewIcon(),
//                debugView.getViewTooltip(), debugPanel);
	...
	if (!showMultipleWallets) {
	    ...
	    if (ComponentOrientation.LEFT_TO_RIGHT == ComponentOrientation.getOrientation(controller.getLocaliser().getLocale())) {
		splitPane.setLeftComponent(null); // depends on orientation
	...
	*/
        ...
        if (showMultipleWallets) fileMenu.add(menuItem);
        ...
        if (showMultipleWallets) fileMenu.addSeparator();
...
//        if (application != null && !application.isMac()) {
	...
	// Send feedback
	menuItem = new JMenuItem(new AbstractAction("Send Feedback") {
	    public void actionPerformed(ActionEvent e) {
		String url = "http://coinspark.org/contact-us/";
		try {
	...
	menuItem.setIcon(ImageLoader.fatCow16(ImageLoader.FATCOW.email));
        menuItem.setFont(FontSizer.INSTANCE.getAdjustedDefaultFont());
        menuItem.setComponentOrientation(componentOrientation);
        helpMenu.add(menuItem);	
...
*/
	...
	showDevelopertoolsAction = new MultiBitWalletBusyAction(this.bitcoinController, 
		ImageLoader.fatCow16Path(ImageLoader.FATCOW.toolbox),
                "showDeveloperToolsAction.text", "showDeveloperToolsAction.tooltip", "showDeveloperToolsAction.mnemonic",
                View.COINSPARK_DEVELOPER_TOOLS_VIEW);
        menuItem = new JMenuItem(showDevelopertoolsAction);
        menuItem.setFont(FontSizer.INSTANCE.getAdjustedDefaultFont());
        menuItem.setComponentOrientation(componentOrientation);
	...
	// Migrate assets
        showMigrateAssetsAction = new MultiBitWalletBusyAction(this.bitcoinController, 
		ImageLoader.fatCow16Path(ImageLoader.FATCOW.baggage_cart_box),
                "showMigrateAssetsAction.text", "showMigrateAssetsAction.tooltip", "showMigrateAssetsAction.mnemonic",
                View.COINSPARK_MIGRATE_ASSETS_VIEW);
        menuItem = new JMenuItem(showMigrateAssetsAction);
        menuItem.setFont(FontSizer.INSTANCE.getAdjustedDefaultFont());
        menuItem.setComponentOrientation(componentOrientation);
//        toolsMenu.add(menuItem);	
//		
//        toolsMenu.addSeparator();
    ...
    private void updateMenuItemsOnWalletChange() {
        ...
        showDevelopertoolsAction.setEnabled(!this.bitcoinController.getModel().getActivePerWalletModelData().isBusy());
        ...
        if (this.bitcoinController.getModel().getActivePerWalletModelData().isBusy()) {
            ...
            showDevelopertoolsAction.putValue(Action.SHORT_DESCRIPTION, walletIsBusyText);
            showMigrateAssetsAction.putValue(Action.SHORT_DESCRIPTION, walletIsBusyText);
	    ...
	    showDevelopertoolsAction.putValue(Action.SHORT_DESCRIPTION, HelpContentsPanel.createTooltipTextForMenuItem(controller.getLocaliser().getString("showDevelopertoolsAction.tooltip")));
            showMigrateAssetsAction.putValue(Action.SHORT_DESCRIPTION, HelpContentsPanel.createTooltipTextForMenuItem(controller.getLocaliser().getString("showMigrateAssetsAction.tooltip")));
        }
    }
	...
	if (this.bitcoinController!=null) assetSummaryTableModel.recreateWalletData();

	// Set the size of the asset summary table.
	Dimension size = assetTableScrollPane.getSize();

	// We no longer want table to resize.
	/*	int gapHeight = 5;
	int n = assetSummaryTableModel.getRowCount();
	if (n > 3) {
	    //assetTableScrollPane.setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS);
	    n = 3; // Restrict height of table
	} else {
	    // TO FIX: Figure out how to get table, viewport, scrollpane height the same, and not have the scrollbar appear.
	    //assetTableScrollPane.setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_NEVER);
	}
*/    
	...
	if (size.height != desiredHeight) {
	    ...
	    assetTableScrollPane.setPreferredSize(size);
	    assetTableScrollPane.setMinimumSize(size);
	    assetTableScrollPane.setMaximumSize(size);
	    assetTableScrollPane.revalidate();
	    this.revalidate();	// need this so other views change position correctly
	}
	// ???
	//assetTable.setAutoResizeMode(JTable.AUTO_RESIZE_OFF); 
//	assetTable.setPreferredScrollableViewportSize(assetTable.getPreferredSize());
//	int y = assetTable.getHeight();
//	log.info("Table: preferred size = " + assetTable.getPreferredSize());
//	log.info("       prefviewportsz = " + assetTable.getPreferredScrollableViewportSize());
//	log.info("Height: scrollPane=" + assetTableScrollPane.getHeight() + " , table height=" + y + " , headerpanel =" + headerPanel.getSize() + "assetSumaryPanel=" + assetSummaryPanel.getSize());
	
	CSBitcoinAssetTableData data = assetSummaryTableModel.getBitcoinAssetTableData();
	data.clear();
	
	// The sync message and percent only gets sent if you open a wallet while MultiBit is syncing.
	// If the wallet was already open, it just shows previous estimated balance.
	    ...
	    data.dataChangedText = controller.getLocaliser().getString("singleWalletPanel.dataHasChanged.text");
	    data.dataChangedToolTipText = getUpdatesStoppedTooltip();
...
//                estimatedBalanceLabelLabel.setToolTipText(null);
		    ...
		    data.syncText = syncMessage;
		    data.syncToolTipText = syncMessage;
		    
//                    estimatedBalanceBTCLabel.setText(syncMessage);
//                    estimatedBalanceBTCLabel.setToolTipText(syncMessage);
                }
                if (syncPercent > -1) {
		    data.syncPercentText = "(" + (int) syncPercent + "%)";
		    data.syncPercentToolTipText = syncMessage;
...
//                availableBalanceFiatButton.setVisible(false);
		...
		data.balanceText = controller.getLocaliser().bitcoinValueToString(estimatedBalance, true, false);
		data.balanceToolTipText = controller.getLocaliser().getString("multiBitFrame.balanceLabel.tooltip");
...
//                    availableBalanceFiatButton.setVisible(false);
		    ...
		    data.spendableText = "(" + controller.getLocaliser().bitcoinValueToString(availableToSpend, true, false) + " spendable)";
...
//            }
	...
	assetSummaryTableModel.fireTableRowsUpdated(0, 0);
    }
    ...
    public void setUpdatesStoppedTooltip(JComponent component) {
        component.setToolTipText(getUpdatesStoppedTooltip());
    }
    
    /*
    @return String for a tooltip if updates have stopped, refactored from setUpdatesStoppedTooltip(component)
    */
    public String getUpdatesStoppedTooltip() {
...
//	log.debug("Received CSEvent: Type=" + event.getType() + " , info=" + event.getInfo());
	final CSEventType t = event.getType();
	...
	if (t == CSEventType.ASSET_VALIDATION_STARTED
		...
		@Override
		public void run() {
		    assetSummaryTableModel.fireTableDataChanged();
		    
		    // Refresh transactions, as a new balance means the coinspark tracking server
		    // has confirmed the transaction and figured out transaction amounts, so the
		    // row for a transaction needs updating.
	    ...
	    final boolean insertedFlag = (t == CSEventType.ASSET_INSERTED);
	    final boolean visibilityFlag = (t == CSEventType.ASSET_VISIBILITY_CHANGED);
	    SwingUtilities.invokeLater(new Runnable() {
		@Override
		public void run() {
		    assetSummaryTableModel.recreateWalletData();
		    updateHeader(); // we want the frame to get bigger as table grows.
		    
		    if (insertedFlag) {
			int modelRow = assetSummaryTableModel.getRowForAssetID(assetId); // use this as no sorter
			assetTable.scrollRectToVisible(assetTable.getCellRect(modelRow,0, true)); 
		    } else if (visibilityFlag) {
			// if invisible, the modelRow will be -1
			int modelRow = assetSummaryTableModel.getRowForAssetID(assetId);
			if (modelRow >= 0) {
			    assetTable.scrollRectToVisible(assetTable.getCellRect(modelRow,0, true)); 			    
			}
		    }
    ...
    public void displayGenesisAssetDialog(CSAsset asset) {
	JOptionPane.showMessageDialog(
			this,
		"Congratulations! Your new CoinSpark asset has been received.\n\n" +
		"To make the asset valid, please upload the provided asset web page\n" +
		"and contract to the directory:\n\n" +
			asset.getAssetWebPageURL() + "\n\n" +
		"Once the asset creation has been fully confirmed (approximately one hour),\n" +
		"the asset can be sent and received freely between CoinSpark wallets.\n\n" +
		"Thank you and please don't hesitate to send feedback via the 'Help' menu.",
		"SparkBit",
			JOptionPane.INFORMATION_MESSAGE,
			ImageLoader.createImageIcon(ImageLoader.MULTIBIT_ICON_FILE)
