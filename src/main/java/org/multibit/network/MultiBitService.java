...
public class MultiBitService {
  ...
  public static final String MULTIBIT_PREFIX = "sparkbit";
...
/* CoinSpark END */
  ...
  public void createNewPeerGroup() {
	...
	// CoinSpark - temporary fix to solve network problem of getting no data returned
  ...
  public static String getFilePrefix() {
    BitcoinController bitcoinController = SparkBit.getBitcoinController();
    // testnet3
    if (TESTNET3_GENESIS_HASH.equals(bitcoinController.getModel().getNetworkParameters().getGenesisBlock().getHashAsString())) {
...
/* CoinSpark START */
          wallet.CS.initCSDatabases(walletFile.getAbsolutePath());
//          wallet.CS.getAssetDB().validateAssets(peerGroup);
//          wallet.CS.getBalanceDB().calculateBalances();
/* CoinSpark END */
	...
	/* CoinSpark START */
	// Set label for the first key in wallet.
	Address defaultReceivingAddress = newKey.toAddress(networkParameters);
